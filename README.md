...
> 版本系列 **Sarastro** · v0.1.0 · 许可 [Apache-2.0](LICENSE)
...
> **状态**：首个公开版（Sarastro 0.1.0），第一期能力已落地；第二、三期见[分期路线](docs/03-phase-plan.md)。
...
| 能力 | 说明 |
|---|---|
| 铭契编辑 | 列表 / 新增 / 修改 / 删除（空文本 = 删除该键）；写前鉴权 + 备份 + 审计 |
| `/epk` 检索可视化 | 复用 EverOS 只读检索，浏览器内预览召回 |
| 画像 / episode 只读 | 分页浏览，鉴权可选（未登录可看） |
...
- **第一期（零 EverOS 耦合）**：铭契编辑 + `/epk` 可视化 + 画像 / episode 只读。✅ 本版
- **第二期（episode 增删改，中耦合）**：改 md + entry → `cascade sync`。
- **第三期（画像编辑，高耦合 + 高风险）**：先做快照 + 只读 diff；编辑会被 flush 回退（治标），YAML 改错致失忆，待评估。

详见 [docs/03-phase-plan.md](docs/03-phase-plan.md)。

> 第二、三期对应的后端模块（`episode_editor` / `profile_viewer`）当前为**有意预留的桩**——真机契约未坐实前不固化签名，非缺失。
